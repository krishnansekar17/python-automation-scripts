...
<div style='text-align: center; margin: 20px 0;'>
    ...
    ✨ Choose the Operator from the Spinning Board to get your dedicated calculator! ✨
    </p>
</div>
""", unsafe_allow_html=True)
...
if st.session_state.selected_operation:
    col_wheel, col_calc = st.columns([1, 1], gap="large")
else:
    col_wheel = st.container()
    col_calc = None

# Spinning wheel (left side)
with col_wheel:
    wheel_html = """
    <style>
        @keyframes spin {
...
# Show calculator (right side)
if st.session_state.selected_operation and col_calc:
    with col_calc:
